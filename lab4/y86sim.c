...
stat_t nexti(y86sim_t *sim)
{
    ...
    switch(icode){
	case I_RRMOVL: case I_IRMOVL: case I_RMMOVL: case I_MRMOVL: case I_ALU: case I_PUSHL: case I_POPL:{
	    if (!get_byte_val(sim->m, next_pc, &codefun)) {
                err_print("PC = 0x%x, Invalid instruction address", sim->pc);
                return STAT_ADR;
    	    }
	...
	{
	    long_t val=0;
	    if(!get_long_val(sim->m, get_reg_val(sim->r, regB)+imm, &val)){
	...
	{
	    long_t valueA = get_reg_val(sim->r, regA);
	    long_t valueB = get_reg_val(sim->r, regB);
	    long_t tem = compute_alu(ifun, valueA, valueB);
	    set_reg_val(sim->r, regB, tem);
	    sim->cc = compute_cc(ifun, valueA, valueB, tem);
	    break;
	}
      case I_JMP: /* 7:x imm */
	{
      ...
      case I_CALL: /* 8:x imm */
	{
	    ...
	    if(!set_long_val(sim->m, addr-4,next_pc)){
      ...
      case I_PUSHL: /* A:0 regA:F */
	{
	    ...
	    set_reg_val(sim->r, REG_ESP, addr-4);
	    if(!set_long_val(sim->m,addr-4,val)){
		err_print("PC = 0x%x, Invalid stack address 0x%x",sim->pc,addr-4);
		return STAT_ADR;
	    }
	    break;
...
{
    FILE *binfile;
    int max_steps = MAX_STEP; //MAX_STEP is 10000
    y86sim_t *sim;            //contains pc, *r, *m, cc
    mem_t *saver, *savem;
    int step = 0;
    stat_t e = STAT_AOK;

    if (argc < 2 || argc > 3)
        usage(argv[0]);		//argc can only be 2 or 3
    ...
    sim = new_y86sim(MEM_SIZE); //pc=0, r 32bytes, m 2^13bytes, cc 1,0,0
