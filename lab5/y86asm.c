...
{
    symbol_t *tem = symtab->next;
    while(tem){
	if(!strncmp(tem->name, name, strlen(name)))
	    return tem;
	tem = tem->next;
    }
    return NULL;
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    int len = 0;
    while(IS_LETTER(cur+len+1) || IS_DIGIT(cur+len+1))
	len++;
    tem = (char*) malloc(len+1);

    //tem[len] = '\0';
    strncpy(tem, cur, len+1);
    cur+=len+1;
...
parse_t parse_digit(char **ptr, long *value)
{
    ...
    if((*cur == '0') && (*(cur+1) == 'x'))
	scale = 16;
    theDigit = strtoll(cur, &tem, scale);
...
parse_t parse_imm(char **ptr, char **name, long *value)
{
    ...
    if(IS_IMM(cur)){
	cur ++;
	parse_t tem = parse_digit(&cur, value);
	if(tem == PARSE_ERR)
	    return PARSE_ERR;
	*ptr = cur;
	return PARSE_DIGIT;
    ...
    if(IS_LETTER(cur)){
	parse_t tem = parse_symbol(&cur, name);
	if(tem == PARSE_ERR)
	    return PARSE_ERR;
	*ptr = cur;
	return PARSE_SYMBOL;
...
int relocate(void)
{
	...
	long *tmp = (long *) &(y86bin->codes[y86bin->bytes - 4]);
	*tmp = sym->addr;
        /* next */
        rtmp = rtmp->next;
    }
...
int binfile(FILE *out)
{
    ...
    while(line){
	if(line->y86bin.bytes != 0){
	    bin_t *y86bin = &line->y86bin;
	    if(tem < y86bin->addr){
		int i;
		int left = y86bin->addr - tem;
		for(i = 0; i<left ; i++)
		    fprintf(out,"%c",0);
	    }
	    tem = y86bin->addr + y86bin->bytes;
